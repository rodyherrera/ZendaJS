    ...
    if(Arguments[0]->IsUndefined())
        Arguments.GetIsolate()->ThrowException(
            String::NewFromUtf8(
                ...
                "In order to execute one more strings of Python code, you must first enter them as a parameter.",
...
static void PythonQuickFile(const FunctionCallbackInfo<Value>& Arguments){
    if(Arguments[0]->IsUndefined())
        Arguments.GetIsolate()->ThrowException(
            String::NewFromUtf8(
                Arguments.GetIsolate(),
                "In order to execute one or more python files you must first indicate them as a parameter.",
                NewStringType::kNormal
            ).ToLocalChecked()
        );
    else{
